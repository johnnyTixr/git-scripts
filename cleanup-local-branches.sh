...
# Cleanup local branches not pushed to remote
...
# Get all local branches not on remote, authored by current user, excluding hotfix and release
echo -e "${BLUE}Fetching local branches not pushed to remote authored by ${YELLOW}${CURRENT_USER}${BLUE}...${NC}"
BRANCHES=()
...
if [ ${#BRANCHES[@]} -eq 0 ]; then
  echo -e "${GREEN}No local unpushed branches found that were authored by you.${NC}"
  exit 0
fi
...
show_menu() {
  clear
  echo -e "${BLUE}═══════════════════════════════════════${NC}"
  echo -e "${BLUE}Local Unpushed Branches Cleanup${NC}"
  echo -e "${BLUE}═══════════════════════════════════════${NC}"
...
confirm_delete() {
  ...
  echo -e "${YELLOW}⚠ This is a local branch that has not been pushed to remote.${NC}"
  echo -e "${RED}This will permanently delete the local branch.${NC}"
